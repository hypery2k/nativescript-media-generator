...
function resize(width, height, bgColour, imagePath, outputFilename, outputPath) {
    ...
    gm(path.join(process.cwd(), imagePath)).size(function(error, size) {
        ...
        if (error) {
            ...
            mkdirp(path.join(process.cwd(), "app", "App_Resources", outputPath), function(err) {
...
function generateImageSets(filename, source, iOSPath) {
    return [
        //IOS Icons
        {
            width: 180,
...
function generate() {
    ...
    fs.readdir(path.join(process.cwd(), "app", "App_Resources", "iOS"), function(err, result) {
